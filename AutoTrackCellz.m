    ...
    cd ..
        ...
        parfor scenenumber = 1:length(sList)
            ...
            idScene = sceneN;
                    ...
                    exportNucleiStructz = nucleiToStructure(idScene,mPath,tFrames,cSeg,nSeg,bSeg,sPath)
                    nucleiStructArray{scenenumber} = exportNucleiStructz;
        end
        
        for scenenumber = 1:length(sList)
            sceneN = sList{scenenumber};
            exportNucleiStruct = nucleiStructArray{scenenumber};
                    ...
                    if isempty(fieldnames(exportNucleiStruct)) %if exportStruct is empty 
                        ...
                        for i = 1:length(plotStruct)
                            for j = 1:length(fnames)
                                exportNucleiStruct(idx+i).(fnames{j}) = plotStruct(i).(fnames{j});
...
%             Tracked = FrickTrackCellsYeah(A,frameToLoad,pvalue,[]);
    ...
    if plottingON == 0
        ...
        framesThatMustBeTracked = psettings.framesThatMustBeTracked;
        if framesThatMustBeTracked(2)+10>timeFrames
        else
