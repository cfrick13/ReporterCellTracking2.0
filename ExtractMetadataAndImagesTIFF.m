function ExtractMetadataAndImagesTIFF(B)
...
cd(PathName)
disp(PathName)
export_stack_dir = 'tifftack images';
...
firstlengths = ceil(fractions);
    ...
    if length(sceneVector)<nWorkers
       nWorkers = length(sceneVector);
    end
...
parfor iNW = 1 : nWorkers
    ...
    for iSeries = sceneArrayVec
        ...
        for iC = 1:channelCount
            ...
            savename = strcat(expName,'_',channelName,'_',sceneNumber,'.tif');
...
%             savefastfilefunc(savename,img_stack_test,'img_stack_test',export_stack_dir,PathName)
            savefastfilefuncTIFF(savename,img_stack_test,'img_stack_test',export_stack_dir,PathName)
%             toc
        end
    end
...
function savefastfilefuncTIFF(savename,img_stack_test,img_stack_str,export_stack_dir,PathName)
cd(export_stack_dir)

imwrite( img_stack_test(:,:,1), savename,'WriteMode','overwrite');
for ind = 2:size(img_stack_test,3)
    imwrite( img_stack_test(:,:,ind), savename,'WriteMode','append');
end
