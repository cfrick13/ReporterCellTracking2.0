...
function plotStruct = plotthemfunctionToStructureLabel(Tracked,idScene,pathStruct,timeFrames,segStruct)
cd(pathStruct.mstackPath)
...
global ImageDetails Tracked segStruct pathStruct MainAxes SceneList timeFrames togStruct psettings
...
idScene = sceneN;
        plotStruct = plotthemfunctionToStructureLabel(trackedArray,idScene,paStruct,tFrames,sStruct);
        plotStructArray{1} = plotStruct;
...
for scenenumber = idxa
    ...
    o = findobj(MainAxes,'Type','Text');
    o.delete;
    
    fnames = fieldnames(plotStruct);        
        idx = 0;
        for i = 1:length(plotStruct)
            centroidvec = plotStruct(i).Centroid;
            centroid = centroidvec(:,:,ImageDetails.Frame);
            cellIDstr = num2str(i);
            cellnumvec = plotStruct(i).CellNum;
            cellnum = cellnumvec(ImageDetails.Frame);
            cellnumstr = num2str(cellnum);
            if ~isnan(cellnum)
                t = text(MainAxes,centroid(1),centroid(2),cellnumstr);
                t.Color = [1 0 0];
                t.FontSize = 20;
            end
        end
...
global  dicbkgmedianmat Ifstack chanbkgmedianmat nucbkgmedianmat togStruct DICimgstack foStruct backgroundimgstack segStruct nucleusimgstack segmentimgstack channelimgstack pathStruct frameToLoad ImageDetails Tracked SceneList imgfile
trackmatrix = Tracked.trackmatrix;
...
cd(pathStruct.mstackPath)
ff = dir(strcat('*',ImageDetails.Scene,'*','647','*'));
if isempty(ff)
...
if isempty(ff)
    ff = dir(strcat('*',ImageDetails.Scene,'*','mKate','*nucleus*'));
end
filename = char(ff.name);
...
else %if no foStruct.cfoName, then5
    backgroundfileObject = matfile(filename);
    backgroundimgstack = backgroundfileObject.IfFinal;
    foStruct.bfoName = char(backgroundfileObject.Properties.Source);%update foStruct.cfoName
end
if updatebkgmedianmat %udpdate background subtraction if new files loaded
    ...
    dicbkgmedianmat = zeros(1,size(backgroundimgstack,3));
...
global mainfig oldscenename togStruct timeFrames tcontrast lcontrast MainAxes ImageDetails prcntl lprcntl cmap cmaplz
...
function makeMovieAllChannelsEGFPplot(~,~)
...
% nannum1 = max([2 find(nanvec == 1,1,'first')]);
nannum1 = 1;
...
f33.Color = 'w';
f33.Units = 'pixels';
f33.Position = [100 100 (xdist*5)+5 ydist];
...
ax6 = axes();
ax6.Units = 'pixels';
ax6.Position = [(xdist*4) 0 xdist ydist];
...
%                 cimg(dispimg>(cimg./a)) = dispimg(dispimg>(cimg./a));
    ...
    if tnum>0
        ...
        if togStruct.displayTrackingToggle==1 && tnum>1
            h = line(ax4,x(:,t0:tnum)',y(:,t0:tnum)','LineWidth',4);
            set(h, {'color'}, num2cell(plotcmap,2));
        end
    end
    ...
    h = plot(ax5,timeVec(nannum1:tnum),plotMat(idx,nannum1:tnum)','LineWidth',2);
    if togStruct.displayTrackingToggle ==1
        for i=1:length(h)
            h(i).Color = cmapl(i,:);
        end
    ...
    ax5.Title.String = ['Nuclear mNG-Smad3 in tracked cells'];
    ax5.FontSize = 12;
    xtick = [0:30:10000]-210;
    ax5.XTick = xtick;
    xticklabel = mat2cell(xtick,1,ones(1,length(xtick)));
    xticklabel(1:2:end) = {''};
    ax5.XTickLabel = xticklabel;
    ytick = [0:0.5:20]-5;
    ax5.YTick = [0:0.5:20]-5;
    ...
    xshift = 30;
    yshift = 40;
    ax5.Position = [(xdist*4)+xshift yshift xdist-xshift-5 ydist-yshift];
    ...
    h = plot(ax6,timeVec(nannum1:tnum),plotMat(idx,nannum1:tnum)','LineWidth',2);
    if togStruct.displayTrackingToggle ==1
        for i=1:length(h)
            h(i).Color = cmapl(i,:);
        end
    ...
    ax6.YLim = (ylimit);
    ax6.YLabel.String = ['Smad3 ' ylabelstr ' ' mmsstr];
    ax6.XLabel.String = 'minutes';
    ax6.XGrid = 'on';
    ax6.YGrid = 'on';
    ax6.Color = [0.95 0.95 0.95];
    ax6.Title.String = ['Nuclear mNG-Smad3 in tracked cells'];
    ax6.FontSize = 12;
    xtick = [0:30:10000]-210;
    ax6.XTick = xtick;
    xticklabel = mat2cell(xtick,1,ones(1,length(xtick)));
    ...
    xshift = 30;
    yshift = 40;
    ax6.Position = [(xdist*4)+xshift yshift xdist-xshift-5 ydist-yshift];
...
%                 cimg(dispimg>(cimg./a)) = dispimg(dispimg>(cimg./a));
    ...
    frametext= text(ax4,0,0,[minstr ' min']);
    frametext.Units = 'pixels';
    frametext.Position = [xdist-5 15];
...
function makeMovieAllChannelsOLD(~,~)
global trackfilestr expDetailsStruct dirStruct timeSteps timeVec pathStruct psettings medmeantot Tracked segStruct Ifstack channelimgstack nucleusimgstack DICimgstack togStruct timeFrames tcontrast lcontrast  ImageDetails prcntl lprcntl cmap cmaplz
...
f33.Color = 'w';
f33.Units = 'pixels';
f33.Position = [100 100 (xdist*4) ydist];
...
function makeMovieCroppedYEAH_RESPONDERS(~,~)
global trackfilestr expDetailsStruct dirStruct timeSteps timeVec pathStruct psettings medmeantot Tracked segStruct Ifstack channelimgstack nucleusimgstack DICimgstack togStruct timeFrames tcontrast lcontrast  ImageDetails prcntl lprcntl cmap cmaplz
%lines for induction
...
%     frametext.Color =[1 1 0];
    ...
    basalValues = plotMat(:,stimulationFrame-3:stimulationFrame-1);
    basalVector = nanmedian(basalValues,2);
    basalMatrix = ones(size(plotMat,2),1)*basalVector';
    pmat = plotMat - basalMatrix';
    
    
    chosencells = pmat>8.32e5;
    chosenidx = sum(chosencells,2)>20;
...
%     ylimit = [prctile(plotMatStat(:),0)*0.95 prctile(plotMatStat(:),100)*1.2];
%     
%     h = plot(ax2,timeVec(nannum1:tnum),plotMat(idx,nannum1:tnum)','LineWidth',2);
%     if togStruct.displayTrackingToggle ==1
%         for i=1:length(h)
%             h(i).Color = cmapl(i,:);
%         end
...
%     end
%     
%     ax2.XLim = timeVec([nannum1 nannum2]);
%     ax2.YLim = (ylimit);
...
%     end
%     
%     ax3.XLim = timeVec([nannum1 nannum2]);
%     ax3.YLim = (ylimit);
...
function makeMovieCroppedYEAHnotSure(~,~)
global trackfilestr expDetailsStruct dirStruct timeSteps timeVec pathStruct psettings medmeantot Tracked segStruct Ifstack channelimgstack nucleusimgstack DICimgstack togStruct timeFrames tcontrast lcontrast  ImageDetails prcntl lprcntl cmap cmaplz
...
function makeMovieCroppedYEAHALT(~,~)
global trackfilestr expDetailsStruct dirStruct timeSteps timeVec pathStruct psettings medmeantot Tracked segStruct Ifstack channelimgstack nucleusimgstack DICimgstack togStruct timeFrames tcontrast lcontrast  ImageDetails prcntl lprcntl cmap cmaplz
...
%FOR IMMUNO!
global channelstoinput ExportName ExportNameKey trackfilestr expDetailsStruct dirStruct timeVec pathStruct psettings medmeantot Tracked segStruct Ifstack channelimgstack nucleusimgstack DICimgstack togStruct timeFrames tcontrast lcontrast  ImageDetails prcntl lprcntl cmap cmaplz
...
function makeMovieCroppedYEAHmultipleScenes(~,~)
%FOR IMMUNO!
global ExportName ExportNameKey trackfilestr expDetailsStruct dirStruct timeVec pathStruct psettings medmeantot Tracked segStruct Ifstack channelimgstack nucleusimgstack DICimgstack togStruct timeFrames tcontrast lcontrast  ImageDetails prcntl lprcntl cmap cmaplz
...
global trackfilestr expDetailsStruct dirStruct timeVec pathStruct psettings medmeantot Tracked segStruct Ifstack channelimgstack nucleusimgstack DICimgstack togStruct timeFrames tcontrast lcontrast  ImageDetails prcntl lprcntl cmap cmaplz
...
%     frametext.Color =[1 1 0];
    ...
    plotMatOG = plotStructUI.(mmsstr).(plotstr);
    plotMatSub = plotMatOG(idx,:);
%     plotMat = smooth(plotMatSub,0.2,'loess');
    plotMat = medfilt1(plotMatSub,3,[],2);
    plotMatStat = plotMat(:,nannum1:nannum2);
    ylimit = [prctile(plotMatStat(:),0)*0.95 prctile(plotMatStat(:),100)*1.2];
...
global trackfilestr dirStruct expDetailsStruct Tracked segStruct Ifstack channelimgstack nucleusimgstack DICimgstack oldscenename togStruct timeFrames tcontrast lcontrast  ImageDetails prcntl lprcntl cmap cmaplz
